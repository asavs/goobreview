...
- Configure the target project through ignored local files copied from `config/*.example.*`: `config/reviewer.env`, `config/project-docs.txt`, `config/head-context-paths.txt`, and `config/required-checks.json`. `scripts/configure.sh` walks the user through these interactively. Personality is the exception — gallery files in `config/personalities/` are committed verbatim and selected via `REVIEWER_PERSONALITY_FILE` in `reviewer.env`.
- The user-facing customization surface is **`config/personalities/<name>.md`** (role, voice, focus areas) selected via `REVIEWER_PERSONALITY_FILE`, and **`config/project-docs.txt`** (repo paths inlined into every prompt). The severity scale (P1/P2/P3) and verdict mapping live in the engine prompt — don't edit `scripts/reviewer/review-prompt.md` unless the user wants to change those contracts or the output format.
