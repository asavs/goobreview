...
if [ -n "$explain" ]; then
  ...
  echo "Deployment policy knobs live in config/reviewer.env: REVIEWER_POSTED_PERSONALITY=${REVIEWER_POSTED_PERSONALITY:-none}, REVIEWER_RESEARCH_CONSENT=${REVIEWER_RESEARCH_CONSENT:-0}, REVIEWER_INCLUDE_AUTHOR=${REVIEWER_INCLUDE_AUTHOR:-0}, REVIEWER_INCLUDE_DESCRIPTION=${REVIEWER_INCLUDE_DESCRIPTION:-0}, REVIEWER_INCLUDE_COMMIT_SUBJECTS=${REVIEWER_INCLUDE_COMMIT_SUBJECTS:-1}."
fi
