...
append_pr_metadata() {
  ...
  metadata=$(github_api_get "repos/$REPO/pulls/$num")
...
write_changed_paths() {
  ...
  github_api_paginate_array "repos/$REPO/pulls/$num/files" |
...
append_diff() {
  ...
  prompt_section "Diff"
  github_api_get "repos/$REPO/pulls/$num" "application/vnd.github.diff"
}
...
build_review_prompt() {
...

  {
    ...
    if prompt_segment_enabled ci_status; then
      append_ci_status "$ci_state" "$num" "$head_sha"
    fi
