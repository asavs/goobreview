...
# Shape:
#   *{model} in [agy {cli_ver}]({agy_release_url}) took {dur} via [goobreview {ref}]({engine_url}).*
# "in agy …" is omitted when the version probe is unavailable; "via goobreview …"
# is omitted when the checkout SHA is unknown. Engine links release notes when
# HEAD is a v* tag tip, else the commit. agy links its release when the probe
# is a clean version tag; otherwise plain "in agy {ver}" with no link.
review_footer_note() {
  local model="$1" elapsed_s="$2" engine_sha="$3"
  local release_tag="${4:-${ENGINE_RELEASE_TAG:-}}"
  local agy_ver="${5:-${AGY_CLI_VERSION:-}}"
  local dur cli_ver agy_url agy_seg="" engine_ref engine_url engine_seg=""
  ...
  if [ -n "$cli_ver" ]; then
    ...
    if [ -n "$agy_url" ]; then
      agy_seg=$(printf ' in [agy %s](%s)' "$cli_ver" "$agy_url")
    else
  ...
  if [ -n "$release_tag" ]; then
    engine_ref="$release_tag"
    engine_url="https://github.com/asavs/goobreview/releases/tag/$release_tag"
    engine_seg=$(printf ' via [goobreview %s](%s)' "$engine_ref" "$engine_url")
