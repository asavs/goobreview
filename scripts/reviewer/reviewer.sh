...
review_one_pr() {
  ...
  local failure_attempts invalid_attempts invalid_artifact ci_state ci_summary
  ...
  case "$ci_state" in
    ...
    failing)
      if [ -n "$DRY_RUN" ] && [ "$DRY_RUN_BYPASS_CI" = "1" ]; then
        ...
        log "PR #$num@$head_sha: CI is failing; no model review will be run or posted"
        ci_summary=$(github_check_runs_summary "$head_sha" 2>>"$LOG_FILE" || true)
        if [ -n "$DRY_RUN" ]; then
          log "Dry run: would mark goobreview check run failed for CI gate on PR #$num@$head_sha; no PR review would be posted"
          review_actions=$((review_actions + 1))
          return 0
        fi
        ...
        conclude_review_check_run_signal failure "CI gate failed; no review posted" \
          "Required CI checks were failing at review time, so GoobReview did not invoke the reviewer model or post a PR review. Fix the failing checks and push a new head SHA for review.${ci_summary:+

${ci_summary}}"
        log "Marked CI gate failure on PR #$num@$head_sha without posting a PR review"
        review_actions=$((review_actions + 1))
        return 0
      fi
      ;;
