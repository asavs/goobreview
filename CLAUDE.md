...
2. **`scripts/reviewer/reviewer.sh`** — the daemon body. Roughly: acquire `flock`; validate `REVIEWER_PERSONALITY_FILE` exists; mint a GitHub App installation token via `get-installation-token.sh` (which `exec`s `lib/app-token.mjs`); list open non-draft PRs; for each PR, skip if already in `seen.txt` or already reviewed by `<app-slug>[bot]` on this head SHA; gate on required CI checks (`check-ci.sh`); download/cache a PR-head source snapshot; assemble a prompt from the personality file, PR diff, and GitHub review formatting rule; run `gemini` from `REVIEWER_STATE/gemini-runtime` with the PR-head snapshot attached as read-only context; parse the first-line GitHub review event; post a top-level review; best-effort apply labels; record `PR_NUMBER HEAD_SHA` in `seen.txt` only after successful post.
...
- **Review event line**: the first line must be exactly `APPROVE`, `REQUEST_CHANGES`, or `COMMENT`. Malformed first lines are dropped and retried next tick. Drift in `review-prompt.md` that changes this format will silently disable every fork.
- **Top-level review body**: everything after the first line is posted as the GitHub review body. Reviews no longer include engine-owned metadata blocks or inline-comment anchors.
